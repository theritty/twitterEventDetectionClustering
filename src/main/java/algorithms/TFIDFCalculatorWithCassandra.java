...
public class TFIDFCalculatorWithCassandra {
     ...
     * @return term frequency of term in document
     */
    public double tf(CassandraDaoHybrid cassandraDao, long round, String country, String term) {
        CountCalculatorHybrid countCalculator = new CountCalculatorHybrid();
        HashMap<String, Double> hm = countCalculator.getCountOfWord(cassandraDao, term, round, country);
        if(hm == null) return 0;
        if(hm.get("totalnumofwords") == 0)
            System.out.println("Term " + term + " total num zero");
     ...
     * @return the inverse term frequency of term in documents
     */
    public double idf(CassandraDaoHybrid cassandraDao, ArrayList<Long> rounds, String country, String term) {
     ...
     * @return the TF-IDF of term
     */
    public double tfIdf(CassandraDaoHybrid cassandraDao, ArrayList<Long> rounds, String term, long roundNum, String country) {
        double tf = tf(cassandraDao, roundNum, country, term) ;
        if(tf == 0)  return 0;
        double idf = idf(cassandraDao, rounds, country,term);
        double result = tf * idf;
